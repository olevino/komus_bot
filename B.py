...
TOKEN = '491188039:AAEvo0T_yDtH4ZR0bHAGfM5lpXYIqlTjcNo'
...
cursor = conn.cursor()
cursor.execute('select ID from antispam')
antispam = list(map(lambda x: int(x[0]), cursor.fetchall()))
...
def logout(message):
    ...
    if (message.chat.id != AdminId):
        bot.send_message(message.chat.id, 'Если вы действительно хотите навсегда удалить себя из памяти бота, обязательно согласуйте это с администратором -> @olevino')
        standart(message)
        return
    ...
    if(id not in list_of_clients):
        bot.send_message('Данный пользователь не авторизован.')
    else:
        conn = sqlite3.connect('Chinook_Sqlite.sqlite')
        cursor = conn.cursor()
        if(id in antispam):
            cursor.execute('delete from antispam where ID=:qw',{'qw':id})
            antispam.remove(id)
        if(id in list_of_clients):
            cursor.execute('delete from list_of_clients where ID=:qw',{'qw':id})
            list_of_clients.remove(id)
        if(id in list_of_vip):
            cursor.execute('delete from list_of_vip where ID=:qw', {'qw': id})
            list_of_vip.remove(id)
        if(id in list_of_Vip):
            cursor.execute('delete from list_of_big_vip where ID=:qw', {'qw': id})
            list_of_Vip.remove(id)
        if(id in list_of_names):
            cursor.execute('delete from list_of_names where ID=:qw', {'qw': id})
            list_of_names.pop(id)
        if(id in last_message):
            cursor.execute('delete from last_message where ID=:qw', {'qw': id})
            last_message.pop(id)
        if(id in cart):
            cursor.execute('delete from cart where ID=:qw', {'qw': id})
            cart.pop(id)
        conn.commit()
        conn.close()
        bot.send_message(message.chat.id, 'Пользователь успешно удалён из базы данных.')


#@bot.message_handler(['restart'])
def restart(message):
    if (message.chat.id != AdminId):
        bot.send_message(message.chat.id, u'У вас недостаточно прав. Забудьте об этой команде:)')
        standart(message)
        return
    os.system('screen python3 A.py')

@bot.message_handler(['broadcast_body'])
def broadcast(message):
    ...
    if (len(message.text.split()) <= 2):
        bot.send_message(message.chat.id,
                         'Введите команду в формате "/broadcast_body ***** ######", где вместо звездочек вставьте ID пользователя, а вместо решеток вставьте сообщение, которое отправится всем клиентам.')
        return
    p = message.text.find('broadcast_body')+15
    a = 0
    while message.text[p] != ' ':
        if(message.text[p] not in '0123456789'):
            bot.send_message(message.chat.id,
                             'Введите команду в формате "/broadcast_body ***** ######", где вместо звездочек вставьте ID пользователя, а вместо решеток вставьте сообщение, которое отправится всем клиентам.')
            return
        a = a*10 +   int(message.text[p])
        p+=1
    p+=1
    ...
    try:
        ...
        bot.send_message(AdminId, 'Не получилось отправить сообщение пользователю '+str(a))
        w=1
    q = message.chat.id
    message.chat.id = a
    if(w==0):
...
def unblock(message):
    if (message.chat.id not in antispam):
        bot.send_message(message.chat.id, 'Вы не отписывались от наших рекомендаций и всё еще на них подписаны')
        standart(message)
        return
    antispam.remove(message.chat.id)
    conn = sqlite3.connect('Chinook_Sqlite.sqlite')
    cursor = conn.cursor()
    cursor.execute('delete from antispam where ID = :ID',{'ID':message.chat.id})
    conn.commit()
    conn.close()
    bot.send_message(message.chat.id, 'Вы успешно подписались на наши рекомендации:)')
    standart(message)
...
def send_users(message):
    ...
    for person in list_of_names:
        if (len(s + list_of_names[person] + '\n') > 4096):
            bot.send_message(AdminId, s)
            s = ''
        s += list_of_names[person] + '\n'
    bot.send_message(AdminId,s)
    time.sleep(0.01)
...
def send_users(message):
    ...
    s = ''
    for person in list_of_names:
        if person in list_of_vip:
            if (len(s + list_of_names[person]+' - vip\n')>4096):
                bot.send_message(AdminId,s)
                s=''
            s+=list_of_names[person]+' - vip\n'
        elif person in list_of_Vip:
            if (len(s + list_of_names[person]+' - VIP\n')>4096):
                bot.send_message(AdminId,s)
                s=''
            s+=list_of_names[person]+' - VIP\n'
...
def send_data_base(message):
    if (message.chat.id != AdminId):
        ...
        return
    try:
        bot.send_document(message.from_user.id, open('Chinook_Sqlite.sqlite', "rb"))
    except Exception as error:
        bot.send_message(message.from_user.id, "Error in sending file" + ": " + str(error))


@bot.message_handler(commands=['gen_invite'])
...
def update_name(message):
    ...
    if(message.chat.id in list_of_clients and (message.chat.id not in list_of_names or list_of_names[message.chat.id] != name) and name != 'SalesKomusBot' and name != 'olevino'):
        conn = sqlite3.connect('Chinook_Sqlite.sqlite')
        cursor = conn.cursor()
...
def hello(message):
    if (message.chat.id not in list_of_clients):
        ...
        return
    bot.send_message(message.chat.id,
                     u"Я - бот, помогающий выбрать интересующий вас товар по низкой цене. Выберите интересующий вас товар на сайте http://komus.ru, а затем дайте мне его артикул. Я расскажу вам по какой цене вы можете его получить и помогу собрать ваш заказ.\n\nОбратите внимание, что мы специализируемся на канцелярских товарах, поэтому не удивляйтесь тому, что большая часть хозяйственных товаров с сайта может оказаться недоступной.\n\nПо всем вопросам пишите тому, от кого вы получили invite-код")
...
def add_code(message):
    ...
    conn.close()
    update_name(message)
    bot.send_message(AdminId,'Зарегистрирован новый пользователь.\nID: '+str(message.chat.id)+'\nHandle: '+list_of_names[message.chat.id])
...
def get_info(a, q=-1):
    ...
    if (float(rozn_price_res)!= -1 and opt_price > rozn_price_with):
        if (q != -1):
            bot.delete_message(q.chat.id, q.message_id)
        bot.send_message(message.chat.id,
    ...
    if (float(rozn_price_res) == -1):
        if (message.chat.id in list_of_Vip):
            text += 'Цена одной пачки: ' + str(round(float(opt_price) * int(float(value)), 2)) + ' руб.\n'
        else:
...
def calc_price(opt_price, rozn_price, id, rozn_price_with):
    ...
    elif (id in list_of_vip or rozn_price==-1):
        p = float(opt_price)*1.3
        if(float(rozn_price)!=-1):
            p = min(p,(float(opt_price) + float(rozn_price)) / 2)
        if(float(rozn_price_with)!=-1):
            p = min(p,float(rozn_price_with))
        return str(p)
    else:
...
def delete_cart_call(message):
    ...
    cursor.execute('insert into last_message values (:ID, :MessageID)',
                   {'ID': message.chat.id, 'MessageID': last_message[message.chat.id]})
    cursor.execute('delete from cart where ID=:id',{'id':message.chat.id})
    conn.commit()
    conn.close()
...
@bot.message_handler(['block'])
def block(message):
    global antispam
    if(message.chat.id in antispam):
        bot.send_message(message.chat.id, 'Вы уже отписаны от рекламной рассылки')
    else:
        antispam+=[message.chat.id]
        conn = sqlite3.connect('Chinook_Sqlite.sqlite')
        cursor=conn.cursor()
        cursor.execute('insert into antispam values (:ID)', {'ID':message.chat.id})
        conn.commit()
        conn.close()
        bot.send_message(message.chat.id, 'Вы успешно отписались от рекламной рассылки')
    standart(message)

@bot.message_handler(['broadcast_spam'])
def broadcast(message):
    if (message.chat.id != AdminId):
        bot.send_message(message.chat.id, 'У вас недостаточно прав. Забудьте об этой команде:)')
        standart(message)
    ...
    if (len(message.text.split()) == 1):
        bot.send_message(message.chat.id,
                         'Введите команду в формате "/broadcast_spam ######", где вместо решеток вставьте сообщение, которое отправится всем клиентам.')
        return
    p = message.text.find('broadcast_spam')
    message.text = message.text[p + 15:]
    for item in list_of_clients:
        if(item not in antispam):
            w=0
            try:
                bot.send_message(item, message.text+'\n\nЕсли вы хотите отписаться от наших рекомендаций, нажмите сюда -> /block')
            except:
                w=1
                bot.send_message(AdminId, 'Не получилось отправить сообщение пользователю ' + str(item))
            q = message.chat.id
...
@bot.message_handler(['broadcast_all'])
def broadcast(message):
    if (message.chat.id != AdminId):
        bot.send_message(message.chat.id, 'У вас недостаточно прав. Забудьте об этой команде:)')
        standart(message)
    ...
    if (len(message.text.split()) == 1):
        bot.send_message(message.chat.id,
                         'Введите команду в формате "/broadcast_all ######", где вместо решеток вставьте сообщение, которое отправится всем клиентам.')
        return
    p = message.text.find('broadcast_all')
    message.text = message.text[p + 14:]
    for item in list_of_clients:
        w=0
        try:
            bot.send_message(item, message.text)
...
def default_handler(a):
    ...
    if (message.text.isnumeric()):
        ...
        get_info(message, q)
    elif 'komus.ru/katalog' in message.text:
        q = bot.send_message(message.chat.id, 'Ожидайте...')
...
bot.send_message(AdminId, "Я родился :)")
while True:
    a = 0
    try:
        bot.polling()
        ...
        if('Read timed out' not in str(error)):
            ...
            time.sleep(10)
    if(a==0):
        bot.send_message(AdminId, "Я умер :( break")
        break
bot.send_message(AdminId, "Я умер :(")
